#!/usr/bin/env bash
#
# erase_events.sh — delete all rows from events.
#
# Empties the events table (TRUNCATE) without dropping it, so the schema and
# indexes stay intact. Use this to clear analysis-generated events (e.g.
# geofence enter/exit) before regenerating them.
#
...
#   POSTGRES_DB=other DB_CONTAINER_NAME=my-db ./erase_events.sh
#
set -euo pipefail
...
for arg in "$@"; do
  case "${arg}" in
    ...
    -h|--help)
      ...
      echo "Deletes all rows from the events table."
      exit 0
      ;;
    *)
...
table_exists="$(docker exec -i "${CONTAINER_NAME}" \
  ...
  -c "SELECT to_regclass('public.events') IS NOT NULL;")"
if [ "${table_exists}" != "t" ]; then
  echo "Table events does not exist. Nothing to erase."
  exit 0
fi
...
if [ "${count}" = "0" ]; then
  echo "events is already empty. Nothing to erase."
  exit 0
fi

# --- Confirmation -----------------------------------------------------------
if [ "${FORCE}" -ne 1 ]; then
  printf "This will delete %s row(s) from events in database '%s'. Continue? [y/N] " \
    "${count}" "${POSTGRES_DB}"
  read -r reply
  case "${reply}" in
    y|Y|yes|YES) ;;
...
echo "Erasing ${count} row(s) from events..."

docker exec -i "${CONTAINER_NAME}" \
  psql -v ON_ERROR_STOP=1 -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" \
  -c "TRUNCATE TABLE events;"

echo "Done. events is now empty."
