#!/usr/bin/env bash
#
# erase_satellites.sh — delete all rows from satellites.
#
# Empties the satellites table (TRUNCATE) without dropping it, so the schema and
# indexes stay intact. Use this to clear seeded satellites before reseeding with
# data_scripts/satellites.sh.
#
...
#   POSTGRES_DB=other DB_CONTAINER_NAME=my-db ./erase_satellites.sh
#
set -euo pipefail
...
for arg in "$@"; do
  case "${arg}" in
    ...
    -h|--help)
      ...
      echo "Deletes all rows from the satellites table."
      exit 0
      ;;
    *)
...
table_exists="$(docker exec -i "${CONTAINER_NAME}" \
  ...
  -c "SELECT to_regclass('public.satellites') IS NOT NULL;")"
if [ "${table_exists}" != "t" ]; then
  echo "Table satellites does not exist. Nothing to erase."
  exit 0
fi
...
if [ "${count}" = "0" ]; then
  echo "satellites is already empty. Nothing to erase."
  exit 0
fi

# --- Confirmation -----------------------------------------------------------
if [ "${FORCE}" -ne 1 ]; then
  printf "This will delete %s row(s) from satellites in database '%s'. Continue? [y/N] " \
    "${count}" "${POSTGRES_DB}"
  read -r reply
  case "${reply}" in
    y|Y|yes|YES) ;;
...
echo "Erasing ${count} row(s) from satellites..."

docker exec -i "${CONTAINER_NAME}" \
  psql -v ON_ERROR_STOP=1 -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" \
  -c "TRUNCATE TABLE satellites;"

echo "Done. satellites is now empty."
