#!/usr/bin/env bash
#
# seed_static_vessel_info_100k.sh — populate static_vessel_info with 100k vessels.
#
# Same as seed_static_vessel_info.sh but generates 100,000 synthetic vessels
# instead of 1,000 (plus the same 10 named seed rows). Idempotent: creates the
# table if needed, upserts the named rows by MMSI, and inserts the synthetic
# rows with ON CONFLICT DO NOTHING, so it can be run repeatedly and layered on
# top of the smaller seed script. Targets the Docker container created by
# deployment_scripts/deploy_database.sh.
#
# Heads-up: 100k rows is a lot more data — ping generation
# (two_weeks_pings_from_now.sh) over this set will be correspondingly heavier.
#
# Override the target via environment variables, e.g.:
#   POSTGRES_DB=other DB_CONTAINER_NAME=my-db ./seed_static_vessel_info_100k.sh
#
set -euo pipefail
...
echo "Seeding static_vessel_info with 100k vessels in database '${POSTGRES_DB}'..."
...
-- 100,000 additional synthetic vessels, generated from lookup arrays so the
-- flag state matches the MMSI country prefix (MID). Deterministic MMSI/IMO
-- (MID*1e6 + g, with g < 1e6 so there are no collisions) keep this idempotent:
-- DO NOTHING leaves any existing rows untouched.
INSERT INTO static_vessel_info
    (mmsi, imo, vessel_name, callsign, flag_state, vessel_type, length_m, width_m, draft_m)
SELECT
